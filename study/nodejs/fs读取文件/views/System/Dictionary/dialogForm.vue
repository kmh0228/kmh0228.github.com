<template>
  <el-form :model="dialogForm" ref="dialogForm" :rules="rule" label-width="120px" label-position="left" class="el-row mes-form-rule">
    <el-form-item label="字典名称" prop="mDataDictTypeName">
      <el-input placeholder="请输入字典名称" size="mini" v-model.trim="dialogForm.mDataDictTypeName"></el-input>
    </el-form-item>
    <el-form-item label="字典类型" prop="mDataDictTypeType">
      <el-input placeholder="请输入字典类型" size="mini" v-model.trim="dialogForm.mDataDictTypeType"></el-input>
    </el-form-item>
...
export default {
  data () {
    return {
      ...
      rule: {
        mDataDictTypeName: [{ required: true, message: '字典名称不能为空' }]
      }
    }
  },
  props: {
