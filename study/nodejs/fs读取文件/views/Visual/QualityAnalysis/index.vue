    ...
    <el-row class="quality-analysis">
      <el-col :span="10" class="quality-num">
        <ul>
          ...
          <li class="num-border">
            <p>良率</p>
            <p :class="passRate>80?'num-normal':'num-error'" >{{passRate+  needSign(passRate)}}</p>
          </li>
        </ul>
      </el-col>
      <el-col :span="6" class="pass-rate split-line">
        <echart-title title="直通率" style="height:20%"></echart-title>
        <rate-pie id="rataPie" style="height:85%" :rateData="rateDataList"></rate-pie>
      </el-col>
      <el-col :span="8" class="trouble-type split-line">
        <echart-title title="不良类别占比" style="height:20%" ></echart-title>
        <pie-echart id="pieEchart" :pieData="pieList" style="height:85%" left="60%"></pie-echart>
      </el-col>
        <el-col :span="16" class="pass-analysis">
          ...
          <quality-analysis id="qualityAnalysis" style="height:85%" :analysisList="analysisList" :dateList="dateList"></quality-analysis>
        </el-col>
        <el-col :span="8" class="bad-ranking split-line">
          <v-title title="不良排名TOP10" titleHight="10%" :subTitle="subTitle"></v-title>
          <rank-bar id="rankBar" :type="errorType" :data="errorData" :height="errorHeight" :rankSum="rankSum" style="height:90%" left="10%"></rank-bar>
        </el-col>
    </el-row>
...
export default {
  ...
  methods: {
    ...
    async getQualityData () {
      let { date } = this
      let res = await this.$api.getQualityData({ date })
      if (res.code === '200') {
    ...
    async getQualityAllData () {
      let { dateType, date } = this
      let data = date ? Object.assign({ dateType, date }) : Object.assign({ dateType })
