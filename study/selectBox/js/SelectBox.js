...
//obj为jquery对象
function SelectBox(obj,data,fn,opts){
	//储存信息
	...
	var fontSize=this.fontSize=opts.fontSize||'14';
	var optionFontSize=opts.optionFontSize||'14';
	...
	var borderColor=opts.borderColor||'#c2c2c2';
	var hoverColor=this.hoverColor=opts.hoverColor||'#6fc';
	var backgroundColor=opts.backgroundColor||'#fff';
	
	var placeholder=this.placeholder=opts.placeholder||'';
	var defalut=this.defalut=opts.defalut||'';
...
SelectBox.prototype.addEvent=function(){
	...
	$ul.on('click','li',function(){
		...
		$input.val(html).attr('index_id',index_id);;
...
SelectBox.prototype.changeList=function(jsons,opts){
	...
	this.input.val(defalut.name).attr('index_id',defalut.id);
	(!this.retur)&&this.fn&&this.fn(defalut);
	
	this.n=0;
	for(i=0;i<length;i++){
		var one=data[i];
		lis+='<li index_id="'+one[dataId]+'">'+one[dataName]+'</li>';		
	}
	$ul.html(lis);
...
SelectBox.prototype.changeFnBack=function(fn){
	this.fnBack=fnBack;
};
...
SelectBox.prototype.setValue=function(name){
