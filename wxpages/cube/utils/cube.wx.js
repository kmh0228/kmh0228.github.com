...
opts:{
	...
	cache:false  是否缓存魔方信息，以便下次打开此浏览器继续
...
function Cube(cont,opts){
	...
	this.cache=typeof this.opts.cache=='undefined'?false:this.opts.cache;//是否缓存步数和颜色
...
Cube.prototype.setCache=function(json){
	if(!this.cache)return;
	for(var name in json){
		this.cacheinfo[name]=json[name];
	}
...
Cube.prototype.initStyle=function(){
	...
	var cacheinfo=this.cacheinfo;
	var x=this.rotateX=cacheinfo.x||0;
	var y=this.rotateY=cacheinfo.y||0;
	var z=this.rotateZ=cacheinfo.z||0;
	...
	for(var i=0;i<this.boxsDataLength;i++){
		...
		this.boxsData[i].dom.style.WebkitTransform='rotateX('+rx+'deg) rotateY('+ry+'deg) rotateZ('+rz+'deg) translate3d('+x+'px,'+y+'px,'+z+'px)';
		for(var j=0;j<6;j++){
			//设置每个面的属性
...
Cube.prototype.setColor=function(colorarr){
	...
	this.setCache({colors:this.colors});
	var b=this.order-1;
	var filter=[{y:0},{y:b},{x:0},{x:b},{z:b},{z:0}];
	//添加颜色
	for(var i=0;i<6;i++){
...
//		_this.oldRotateZ=_this.rotateZ;
		...
		_this.setCache({x:_this.rotateX,y:_this.rotateY,z:_this.rotateZ});
...
Cube.prototype.turn=function(coor,num,dir,fnComplete){
	...
	//累计步数
	this.foots++;
	this.setCache({foots:this.foots});
	//找到操作的元素
	var filter={};
	filter[coor]=num;
	...
	function transend(){
		...
		//全部旋转完成后
			...
			for(var i=0;i<domLength;i++){
				dom[i].dom.style.WebkitTransform='rotateZ(0deg) rotateY(0deg) rotateX(0deg) translate3d('+dom[i].translateX+'px,'+dom[i].translateY+'px,'+dom[i].translateZ+'px)';
			}
			_this.record.push({coor:coor,num:num,dir:dir});
			_this.runing=false;
			_this.turnBack&&_this.turnBack({coor:coor,num:num,dir:dir});
			_this.success&&(function(){
				var colors=_this.colors;
				var suc=true;
				for(var i=0;i<colors.length;i++){
					var colorsface=colors[i];
					for(var j=1;j<colorsface.length;j++){
						if(colorsface[j]!=colorsface[j-1]){
							suc=false;break;
						}
					}
				}
				console.log(suc);
				suc&&_this.success.call(_this);
			})();
	...
	for(var i=0;i<domLength;i++){
		...
		dom[i].dom.style.WebkitTransform='rotateZ('+drz+'deg) rotateY('+dry+'deg) rotateX('+drx+'deg) translate3d('+dom[i].translateX+'px,'+dom[i].translateY+'px,'+dom[i].translateZ+'px)';
	}
};
...
Cube.prototype.back=function(){
	var _this=this;
	var json=this.record.pop();
	if(json){
		this.turn(json.coor,json.num,!json.dir,function(){
			_this.record.pop();
		});
	}
};
